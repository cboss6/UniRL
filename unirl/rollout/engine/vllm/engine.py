...
class VLLMRolloutEngine(BaseRolloutEngine):
    ...
    def update_weights_from_tensor(
        ...
        *,
        ...
        if not self._is_tp_zero:
            return
        payloads = list(serialized_named_tensors)
        if len(payloads) != self._tp_size:
            raise ValueError(
                f"direct vLLM weight payload count {len(payloads)} "
                f"does not match rollout TP size {self._tp_size}"
            )
        self._request(
            "update_weights",
        ...
        self._version += 1
        logger.info(
            "Direct vLLM TP%d weight update completed: version=%d payloads=%d",
            self._tp_size,
            self._version,
            len(payloads),
        )
    ...
    def _truncate_prepared_prompts(self, prepared: Any) -> None:
        ...
        for index, (payload, prompt_token_ids) in enumerate(
            zip(prepared.wire, prepared.prompt_token_ids, strict=True)
        ):
