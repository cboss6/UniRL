        ...
        for completion in sorted(request.outputs, key=lambda item: int(item.index)):
            ...
            if completion.logprobs is None:
                ...
                for token_id, choices in zip(token_ids, completion.logprobs, strict=True):
                    ...
                    if selected is None:
                        raise RuntimeError(
                            f"vLLM omitted sampled token {token_id} from processed logprobs"
                        )
    ...
    *,
    ...
    try:
        ...
        if os.environ.get("UNIRL_WEIGHT_DEBUG", "0") == "1":
            print(
                f"[unirl.vllm.weight.debug] "
                f"{llm.collective_rpc('unirl_weight_debug')}",
                flush=True,
            )
    ...
    try:
        while True:
            ...
            try:
                if command == "generate":
                    if (
                        os.environ.get("UNIMATCH_ATTN_DEBUG", "0") == "1"
                        or os.environ.get("UNIMATCH_ROUTER_DEBUG", "0") == "1"
                        or os.environ.get("UNIMATCH_LM_DEBUG", "0") == "1"
                        or bool(os.environ.get("UNIMATCH_STAGE_DUMP_DIR"))
                    ):
                        llm.collective_rpc(
                            "unirl_begin_debug",
                            kwargs={
                                "record_id": str(
                                    message.get("record_id", "batch")
                                )
                            },
                        )
                    ...
                    prompts = [
                        {"prompt_token_ids": [int(token) for token in payload["input_ids"]]}
